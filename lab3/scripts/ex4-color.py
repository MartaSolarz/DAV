...
def main():
    ...
    for i, task in tasks.iterrows():
        ...
        if pd.isnull(task['finish_date']):
            ax.plot([start, start], [y_position - 0.1, y_position + 0.1], color="tab:red", linewidth=2)
            pos = -0.5
            loc = 'center'
            ...
            try:
                color = code_to_color[labels_for_axis[task['description']]]
            except KeyError:
                color = "grey"
            ...
            if (txt == 'I' or txt == 'II') and labels_for_axis[task['description']] == 'holidays':
                pos = -0.5
                ax.plot([start, finish], [y_position, y_position], color='blue', marker='o', linewidth=3, markersize=2)
                ax.text(middle, y_position + pos, txt, fontsize=12, ha='center', va='bottom')
                continue

            ax.plot([start, finish], [y_position, y_position], color=color, marker='o', linewidth=8, markersize=10)
            ax.text(middle, y_position + pos, txt, fontsize=12, ha='center', va='bottom')
    ...
    fig.savefig("../plots/ex4-color.pdf", bbox_inches='tight')
